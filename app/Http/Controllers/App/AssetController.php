...
class AssetController extends Controller
{
    ...
    /**
     * Test email image function - sends a simple test email to luke@angelfs.com
     */
    public function testEmailImage()
    {
        try {
            // Create a dummy user object for luke@angelfs.com
            $testRecipient = new \stdClass();
            $testRecipient->email = 'luke@angelfs.com';
            $testRecipient->name = 'Luke';

            // Create dummy data for the notification
            $dummyReturnRecord = new \stdClass();
            $dummyReturnRecord->datetime = now();
            $dummyReturnRecord->notes = 'Test email for image display';

            $dummyKit = new \stdClass();
            $dummyKit->alias = 'TEST-KIT-001';

            $dummyReturnedToUser = new \stdClass();
            $dummyReturnedToUser->name = 'Test User';
            $dummyReturnedToUser->firstname = 'Test';
            $dummyReturnedToUser->surname = 'User';
            $dummyReturnedToUser->hr_id = 'HR001';

            $dummyProcessedByUser = new \stdClass();
            $dummyProcessedByUser->name = auth()->user()->name ?? 'Test Admin';
            $dummyProcessedByUser->firstname = auth()->user()->employee->firstname ?? 'Test';
            $dummyProcessedByUser->surname = auth()->user()->employee->surname ?? 'Admin';
            ...
            // Convert to User model for notification
            $user = User::where('email', 'luke@angelfs.com')->first();
            if (!$user) {
                // Create a temporary user object that can receive notifications
                $user = new User();
                $user->email = 'luke@angelfs.com';
                $user->name = 'Luke';
            }

            // Send the test notification
            $user->notify(new EquipmentReturnNotification(
                $dummyReturnRecord,
                $dummyKit,
                $dummyReturnedToUser,
                $dummyProcessedByUser,
                $dummyReturnItems,
                $dummyAttachments
            ));

            \Log::info('Test email sent successfully to luke@angelfs.com');
            return response()->json(['message' => 'Test email sent successfully to luke@angelfs.com'], 200);
