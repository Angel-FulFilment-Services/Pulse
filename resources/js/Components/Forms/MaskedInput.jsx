...
export default function MaskedInput(props) {
  ...
  return (
    <div>
      {(label || annotation) && <label htmlFor={id} className="block text-sm font-medium leading-6 text-gray-900 mb-2">
        {label}
      ...
      <div className="">
        <div className={`relative flex rounded-md shadow-sm ring-1 ring-inset ${error ? 'ring-red-600 text-red-800' : 'ring-gray-300'} focus-within:ring-2 focus-within:ring-inset focus-within:ring-theme-600 sm:max-w-md h-full`}>
          <InputMask
            ...
            id={id}
            className={`block flex-1 border-0 bg-transparent py-1.5 pl-3 ${error ? 'text-red-800' : 'text-gray-900'} placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6 focus:outline-none placeholder:normal-case ${uppercase ? 'uppercase' : ''}`}
