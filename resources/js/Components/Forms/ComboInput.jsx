...
export default function ComboInput(props) {
  const { id, label, currentState, items, placeholder, spellCheck, onComboChange } = props;
    ...
    <>
      <Combobox as="div" value={currentState} onChange={ e => { setSelected(e); handleComboChange(e);}}>
        ...
        <div className="relative mt-2">
          <Combobox.Input
            className={`w-full rounded-md border-0 bg-white py-1.5 pl-3 pr-10 z-10 text-gray-900 ${typeof selected !== "undefined" && !selected.value && "text-gray-400"} shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:outline-none focus:ring-orange-600 sm:text-sm sm:leading-6`}
