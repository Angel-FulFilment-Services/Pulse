...
export default function ReportingTargets({ column, targets, handleTargetChange }) {
  ...
  return (
    <div className="w-full mx-auto flex flex-col space-y-2 divide-y divide-gray-300 dark:divide-dark-600 cursor-auto">
      <div className="w-full flex justify-between items-center gap-x-6">
        <div className="w-full flex items-center justify-start">
          <h1 className="text-sm text-gray-600 dark:text-dark-200"> Target Direction </h1>
        </div>
        <div className="w-1 h-1 flex-shrink-0 rounded-full bg-gray-300 dark:bg-dark-600"></div>
      ...
      <div className="w-full flex justify-between items-center gap-x-6 pt-2">
        <div className="w-full flex items-center justify-start">
          <h1 className="text-sm text-gray-600 dark:text-dark-200"> Target <span className="text-gray-400 dark:text-dark-500">(High)</span> </h1>
        </div>
        <div className="w-1 h-1 flex-shrink-0 rounded-full bg-gray-300 dark:bg-dark-600"></div>
        <div className="w-full flex items-center justify-end gap-x-1">
          ...
          <XMarkIcon className="w-5 h-5 text-gray-400 dark:text-dark-500 cursor-pointer" onClick={(value) => handleTargetChange({id: column.id, key: "high", target: null})}/>
        </div>
      </div>
      <div className="w-full flex justify-between items-center gap-x-6 pt-2">
        <div className="w-full flex items-center justify-start">
          <h1 className="text-sm text-gray-600 dark:text-dark-200"> Target <span className="text-gray-400 dark:text-dark-500">(Low)</span> </h1>
        </div>
        <div className="w-1 h-1 flex-shrink-0 rounded-full bg-gray-300 dark:bg-dark-600"></div>
        <div className="w-full flex items-center justify-end gap-x-1">
          {column.dataType == 'Currency' ? 
          ...
          <XMarkIcon className="w-5 h-5 text-gray-400 dark:text-dark-500 cursor-pointer" onClick={(value) => handleTargetChange({id: column.id, key: "low", target: null})}/>
        </div>
      </div>
    </div>
  );
