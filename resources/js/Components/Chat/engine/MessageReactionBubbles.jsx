...
export default function MessageReactionBubbles({ reactions, isMyMessage, currentUserId, onRemoveReaction, messageId, onHoverChange, boundaryRef, pendingReactionsRef }) {
  ...
  React.useEffect(() => {
    ...
    const previousEmojis = new Set(Object.keys(previousReactionCountsRef.current))
  ...
  const handleClick = (group) => {
    ...
    const reactionKey = `${messageId}-${currentUserId}-${group.emoji}`
    ...
    if (group.count === 1) {
      ...
      onRemoveReaction(messageId, { emoji: group.emoji, name: group.name })
